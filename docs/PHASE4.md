...
| **Where a cell's behaviour comes from** | `behaviour.rs`, `Behaviour::contract` | The **first gene whose `trigger_state` matches the cell's state**, which is development's own rule with the step window taken off. SPEC never says. ⚠️ **Reversed in Phase 7's Group I**, and the row is kept as written because the reasoning is worth re-reading against what happened: measured, that lookup connected **2.2% of grown cells** to their own genome and **not one myocyte in the world**. A cell now takes its behaviour from the gene that *built* it. See `docs/PHASE7.md` Group I and SPEC section 7. |
