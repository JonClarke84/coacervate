...
mod tests {
    ...
    /// ⚠️ **DIAGNOSTIC, not a gate.** What `what_moving_is_worth_per_tick` is actually reading,
    /// split into the part that is field structure and the part that is max-of-sixteen.
    ///
    /// Three centres for the same ring, on the same settled field:
    ///
    /// - **bodies** — the instrument's own centres,
    /// - **same depth, random column** — destroys where a body is without touching the depth
    ///   gradient, which is the one piece of structure that is certainly real,
    /// - **uniformly random** — destroys depth as well.
    ///
    /// And three readings off each ring: `max − here` (what the instrument uses), `median − here`
    /// (what moving in a direction you did not choose is worth) and `max − median` (**the price of
    /// perfect aim, which is exactly the max-of-sixteen selection**).
    #[test]
    #[ignore = "a settled world per profile; run deliberately with --ignored"]
    fn the_honest_null_for_what_moving_is_worth() {
        const BEARINGS: usize = 16;
        const REACHES: [f32; 4] = [16.6, 88.0, 232.0, 500.0];
        ...
        /// The field as a photocyte's income actually feels it: the mean over a box of tiles
        /// `radius` wide, which is the screening length `tile × sqrt(diffusion / uptake)` of the
        /// crater a body digs. A radius of nought is one bare tile, which is what the instrument
        /// reads. Wraps sideways and is closed top and bottom, as the grid is.
        fn smoothed(grid: &coacervate_sim::grid::Grid, at: Vec2, radius: usize) -> f64 {
            let tiles = grid.tiles();
            let index = grid.tile_at(at);
            if radius == 0 {
                return f64::from(tiles[index]);
            }

            let (cols, rows) = (grid.cols(), grid.rows());
            let (col, row) = (index % cols, index / cols);
            let (mut sum, mut count) = (0.0, 0u32);
            for down in 0..=(2 * radius) {
                if row + down < radius || row + down - radius >= rows {
                    continue;
                }
                let there = row + down - radius;
                for across in 0..=(2 * radius) {
                    let over = (col + cols + across - radius) % cols;
                    sum += f64::from(tiles[there * cols + over]);
                    count += 1;
                }
            }
        ...
        /// The instrument's own ring, sample for sample, plus what it threw away.
        fn ring(
            grid: &coacervate_sim::grid::Grid,
            at: Vec2,
            reach: f32,
            width: f32,
            height: f32,
            radius: usize,
        ) -> ([f64; BEARINGS], f64, usize, usize) {
            let here = smoothed(grid, at, radius);
            let mut seen = [0.0f64; BEARINGS];
            let mut clamped = 0usize;
            let mut best = 0usize;
            let mut highest = f64::NEG_INFINITY;

            for (step, slot) in seen.iter_mut().enumerate() {
                let turn = f32::from(u16::try_from(step).expect("sixteen fits in a u16"));
                let (sin, cos) = (turn * std::f32::consts::TAU / 16.0).sin_cos();
                let raw_y = at.y + sin * reach;
                if raw_y < 0.0 || raw_y > height {
                    clamped += 1;
                }
                let there = Vec2::new(
                    (at.x + cos * reach).rem_euclid(width),
                    raw_y.clamp(0.0, height),
                );
                *slot = smoothed(grid, there, radius);
                if *slot > highest {
                    highest = *slot;
                    best = step;
                }
            ...
            (seen, here, clamped, best)
        }

        /// `max`, `median` and `mean` of one ring.
        fn read(seen: &[f64; BEARINGS]) -> (f64, f64, f64) {
            let mut sorted = *seen;
            sorted.sort_by(f64::total_cmp);
            let max = sorted[BEARINGS - 1];
            let median = 0.5 * (sorted[BEARINGS / 2 - 1] + sorted[BEARINGS / 2]);
            let mean = seen.iter().sum::<f64>() / 16.0;
            (max, median, mean)
        }

        let probe = |what: &str, uptake: f64, diffusion: f64| {
            let settings = config(|raw| {
                raw.light.uptake = uptake;
                raw.light.diffusion = diffusion;
            });
            ...
            let (width, height) = (settings.world.width, settings.world.height);
            let grid = world.grid();
            let cells: Vec<Vec2> = world.living_cells().iter().map(|cell| cell.pos).collect();
            let n = f64::from(u32::try_from(cells.len().max(1)).expect("a cell count"));

            println!(
                "\n================ {what} (uptake {uptake}, diffusion {diffusion}) ================"
            );
            println!("living cells {}", cells.len());

            // ---- the field itself, before anything is sampled at a body ----
            let tiles = grid.tiles();
            let (cols, rows) = (grid.cols(), grid.rows());
            let count = f64::from(u32::try_from(tiles.len()).expect("a tile count"));
            let mean_tile = tiles.iter().map(|t| f64::from(*t)).sum::<f64>() / count;
            let var_tile = tiles
                .iter()
                .map(|t| (f64::from(*t) - mean_tile).powi(2))
                .sum::<f64>()
                / count;

            // Row means, so the depth gradient can be taken out and the *horizontal* structure
            // looked at on its own.
            let mut row_mean = vec![0.0f64; rows];
            for (row, slot) in row_mean.iter_mut().enumerate() {
                let across = &tiles[row * cols..(row + 1) * cols];
                *slot = across.iter().map(|t| f64::from(*t)).sum::<f64>()
                    / f64::from(u32::try_from(cols).expect("a column count"));
            }
            let residual: Vec<f64> = (0..tiles.len())
                .map(|index| f64::from(tiles[index]) - row_mean[index / cols])
                .collect();
            let res_var = residual.iter().map(|r| r * r).sum::<f64>() / count;

            println!(
                "field: mean tile {mean_tile:.4}, sd {:.4}; after removing the row mean, sd {:.4} \
                 (so {:.1}% of the field's variance is depth)",
                var_tile.sqrt(),
                res_var.sqrt(),
                100.0 * (1.0 - res_var / var_tile.max(f64::MIN_POSITIVE))
            );

            print!("horizontal autocorrelation of the residual, by lag (tiles/units): ");
            for lag in [1usize, 2, 3, 4, 8, 16, 32, 64] {
                let mut sum = 0.0;
                for row in 0..rows {
                    for col in 0..cols {
                        sum +=
                            residual[row * cols + col] * residual[row * cols + (col + lag) % cols];
                    }
                }
                let corr = (sum / count) / res_var.max(f64::MIN_POSITIVE);
                print!(
                    "{lag}/{:.0}={corr:+.3}  ",
                    f64::from(u32::try_from(lag).unwrap()) * 8.0
                );
            }
            println!();

            // ---- the three sets of centres ----
            let mut stir = Stir(0x00c0_ffee_u64);
            let mut random_at_depth: Vec<Vec2> = Vec::with_capacity(cells.len());
            let mut random_anywhere: Vec<Vec2> = Vec::with_capacity(cells.len());
            for cell in &cells {
                random_at_depth.push(Vec2::new(stir.unit() * width, cell.y));
                random_anywhere.push(Vec2::new(stir.unit() * width, stir.unit() * height));
            }

            // The screening length of the crater a body digs: `tile × sqrt(diffusion / uptake)`,
            // rounded to whole tiles. It is the width over which the field an income actually
            // feels is averaged — a single-tile excursion is levelled into its neighbours in
            ...
            println!(
                "screening length {screening:.2} world units ({:.2} tiles); crater relaxes in \
                 {:.2} ticks",
                screening / tile_width,
                1.0 / (uptake + 4.0 * diffusion)
            );
            ...
            let mut spread = 0.0;
            for at in &cells {
                let (seen, here, _, _) = ring(grid, *at, 16.6, width, height, 0);
                let (max, _median, _mean) = read(&seen);
                if (max - here).abs() <= 1e-5 {
                    ties += 1.0;
                }
            ...
            println!(
                "at reach 16.6: the ring's max is within 1e-5 of `here` for {:.2}% of bodies; \
                 mean max-min across the ring {:.5}",
                100.0 * ties / n,
                spread / n
            );
        };

        probe("the shipped world", 0.01, 0.04);
        probe("the best world", 0.90, 0.002);
    }
