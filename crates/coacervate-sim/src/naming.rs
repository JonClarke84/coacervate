...
/// how well it does it.
/// ⭐ **Phase 7 Group C added seven and widened one**, and every one of them is a word the
/// syllable tables cannot spell but the *event log* can write in a sentence. `succeeded` does not
/// contain `success`; `worst` was banned and `worse` was not; and there is no `y`, `w`, `ie`, `iu`
/// or `oa` anywhere in the tables, so `trying`, `toward`, `achiev`, `triumph`, `goal` and `ladder`
/// were never going to be reached by a *name* and were therefore never noticed to be missing. No
/// name generated by any earlier version of this project is changed by their being here.
const FORBIDDEN: [&str; 85] = [
    ...
    "advanc", "progress", "improv", "better", "best", "wors", "success", "succeed", "fail",
    "primitiv", "perfect", "evolv", "regress", "elit", "higher", "trying", "toward", "achiev",
    "triumph", "goal", "ladder",
...
mod tests {
    ...
    /// ⭐ **Phase 7, `C9`.** Every phrase CLAUDE.md bans outright is refused by this one list.
    ///
    /// > Never *"evolving toward"*, *"progress"*, *"advanced"*, *"primitive"*, *"higher"*,
    /// > *"improved"*, *"better"*, *"more evolved"*, *"trying to"*, *"succeeded in"*.
    ///
    /// ⚠️ **Two of the ten were not caught, and the near-miss is the reason this test is written
    /// out phrase by phrase rather than trusted.** *succeeded* does not contain *success* — the
    /// letters differ from the fifth onwards — and there is no `y` anywhere in [`ONSETS`] or
    /// [`NUCLEI`], so *trying* was never going to be spelled by the syllable tables and was
    /// therefore never noticed to be missing. Group B's list was written against what a *name*
    /// could accidentally say; Group C's log is written in English, where both of them are one
    /// slip away.
    ///
    /// [`is_permissible`] is deliberately the only list. `docs/PHASE7.md`: *"two such lists are
    /// two lists that come apart, and the register of the log and the register of the names are
    /// one register."*
    #[test]
    fn the_banned_vocabulary_is_claude_mds_whole_list() {
        for phrase in [
            "evolving toward",
            "progress",
            "advanced",
            ...
            assert!(
                !is_permissible(phrase),
                "\"{phrase}\" is one of the ten CLAUDE.md bans by name, and this project would \
                 print it"
            );
        }
    }
