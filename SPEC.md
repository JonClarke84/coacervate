...
| `tempo` | ⭐⭐⭐ **The shipped world living its life in eight times fewer ticks** — `metabolism.upkeep_scale` 1.0 → 8.0, `light.influx` 0.001 → 0.008 and `light.cap` 8.0 → 64.0, which is every per-tick **rate** multiplied by eight and every **stock** left alone. It is the only lever this project has found on *how many ticks a generation costs*, and that is the number that decides how much evolution a night of wall clock buys. **Mean age 928 → 109 ticks**, per-capita birth rate ×8, and ⭐ **biomass and total living cells do not move** — 33,182 → 32,472 and 5,236 → 5,089 across the whole range. The world holds the same amount of life and turns it over eight times faster. ⚠️⚠️ **`dt` is not this lever and cannot be**: `DT` is read by the integrator, by detritus sinking and by a myocyte's phase clock, and by nothing in `metabolism.rs`, `reproduction.rs`, `ledger.rs`, `grid.rs` or `organism.rs` — upkeep is charged per tick and a lifespan is a tick count. Measured: an **eightfold `dt` moved mean age from 881 to 956**, which is 8% and the wrong way. ⚠️⚠️ **The price is the body-size distribution**: the mean body halves, 8.17 cells → 3.87, with the population doubling — the same tissue differently divided — and the step falls between ×1 and ×2 rather than spreading across the range, which reads like a discretisation threshold nobody has yet identified. Do not read a body-size result off this profile until somebody has. ⚠️ No coefficient recorded anywhere in this document was taken in this world. `docs/NEXT.md` section 7 carries the tables. |
